            ...
            <div className="info-item full-width alert-block">
              ...
              <div className="info-value">
                ...
                {Number.isFinite(Number(alert.idleDuration)) && (
                  <div><strong>Idle:</strong> {formatSecondsToHHMM(alert.idleDuration)}</div>
                )}
                {(() => {
                  const uid = alert.userId || alert.uid || '';
                  const hideUid = uid === 'ZsWMn71fYrY2Z8qSCb4qxN7O4XC2';
                  return !hideUid && uid ? <div><strong>UID:</strong> {uid}</div> : null;
                })()}
                <div><strong>Location:</strong> {alert?.location?.latitude ?? '-'}, {alert?.location?.longitude ?? '-'}</div>
              </div>
            </div>
          )}
    ...
    <div className="dashboard-container">
      <div className="dashboard-header">
        <div className="header-content">
          ...
          <p className="section-subtitle">Auto-detected safety alerts and missing complaints</p>
        </div>
      ...
      {loading && (
        <div className="skeleton-grid">
          {Array.from({ length: Math.max(3, Math.min(8, items.length || 6)) }).map((_, i) => (
      ...
      {items.length === 0 && !loading ? (
        <div className="empty-state">
          ...
          <p>No missing complaints or safety alerts found.</p>
        </div>
      ) : (
